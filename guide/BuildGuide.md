...
- ミニホットプレートにトラパPCBを載せて加熱＆ICの四隅や上面からはんだごてで加熱を助けます。
- 十分に加熱されるとICが動くので、ピンセットでICをとりはずし、はんだ吸い取り線でPCBやICの余剰はんだを除去します。※単なるはんだクラックであれば、加熱だけで治るケースも。
- 一度ホットプレートからPCBを下して、はんだペーストを適量PCBのパターンに塗布します。
- ICの向き（PCBとICにプリントされた丸いドットの位置を合わせる）に注意しつつ、ピンセットでPCBのパターンにICを載せます。
- 再びホットプレートでPCBを加熱し、はんだペーストが解けてICがニュルンと表面張力で動いたら完了。
- ホットプレートからPCBを下して、トラパケースにトラパPCBをセットし、キーボードと接続して動作確認します。
